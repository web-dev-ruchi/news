...
include "config.php";
    ...
    if (mysqli_num_rows($result) > 0) {
        echo "<p style='color:red;text-align:center;margin:10px 0;'>Username already use.</p>";
    } else {
        $query2 = "INSERT INTO user (first_name,last_name,username,password,role)
        VALUES ('{$fname}','{$lname}','{$user}','{$password}','{$role}')";
        $result2 = mysqli_query($conn, $query2) or die("Query Failed2!");
        if ($result2) {
